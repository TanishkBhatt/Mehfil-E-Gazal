...
with st.form(key="get-song"):
    song = st.selectbox(
        "SELECT THE SONG NAME YOU WANNA GO THROUGH",
...
if get_song:
    ...
    with st.container(border=True):
        reccomended_by_singer, reccomended_by_category = recommend_more("datasets/songs.json", data["Singer"], data["Category"])
        reccomended_by_singer.remove((song, data["Singer"]))

        st.markdown("##### FROM THE SAME SINGER")
        st.markdown("")

        if reccomended_by_singer:
            for rec_song in reccomended_by_singer:
                try:
                    st.markdown(f"{rec_song[0]}")
    ...
    with st.container(border=True):
        ...
        if reccomended_by_category:
            for rec_song in reccomended_by_category:
                try:
                    st.markdown(f"{rec_song[0]}")
                    st.audio(f"datasets/audio/{rec_song[0]}.mp3")
                except:
